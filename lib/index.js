/**
 * dsh-rescue-bootloader — DSH 救砖模块（Python 实现，v1.1.1 一条命令安装/卸载）
 *
...
function pythonWorks(cmd, prefix) {
  try {
    const r = spawnSync(cmd, prefix.concat(['-c', 'import sys;print(1)']), { timeout: 15000, encoding: 'utf8' });
...
function discoverPython() {
  ...
  for (const item of cands) {
    ...
    let exe = '';
    try {
      const r = spawnSync(cmd, prefix.concat(['-c', 'import sys;print(sys.executable)']), { timeout: 15000, encoding: 'utf8' });
