...
/** 读取安装时记录的 python 路径（setup.py 写入），回退到 PATH 上的 'python' */
function getPython() {
  try {
    const pf = path.join(PLUGIN_ROOT, 'data', 'python.path');
    if (existsSync(pf)) {
      const v = readFileSync(pf, 'utf8').trim();
      if (v) return v;
    }
  } catch {}
  return 'python';
...
function startRescueServer() {
  ...
  const child = spawn(getPython(), [SERVER_SCRIPT, String(RESCUE_PORT), PROFILE_DIR, RESCUE_DIR], {
    detached: true,
    stdio: 'ignore',
  });
  child.unref();
