...
## 1.1.5 (安全模式误判修复)
- **修复(误判)**：`dsh_rescue.py` 的启动检测把「启动超时（DSH 进程仍存活，多为插件初始化/网络阻塞导致的慢启动）」与「真实崩溃（进程退出）」混为一谈——超时也会进入安全模式并裁剪插件，导致正常慢启动被误裁掉 16 个插件。现改为**仅当 DSH 进程真实退出**才进入安全模式；「超时但进程存活」判为慢启动：保留全部插件、仅提示并继续挂住监听。
- **调整**：默认启动检测超时由 60 秒提高至 **300 秒**，给重型全量启动（含 WebUI/市场/通知等大批插件）留足时间。
